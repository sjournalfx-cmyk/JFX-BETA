...
import { 
  ...
  Hash, Image as ImageIcon, Mic, Save, ChevronRight, ChevronLeft,
  TrendingUp, TrendingDown, Layout, Type,
  CheckCircle2, XCircle, MinusCircle, Upload, FileText,
  ArrowRight, Brain, AlertTriangle, ShieldCheck, Check, ChevronDown, X, Star, Eye, Trash2
} from 'lucide-react';
...
const StyledTextArea = ({ isDarkMode, className = "", ...props }: any) => (
    <textarea 
        {...props}
        className={`w-full bg-transparent p-4 min-h-[120px] rounded-lg border outline-none resize-none text-sm leading-relaxed transition-all focus:ring-4 focus:ring-violet-500/5
            ${isDarkMode 
                ? 'bg-[#18181b] border-[#27272a] text-zinc-200 placeholder-zinc-600 focus:border-violet-500/50' 
                : 'bg-white border-slate-200 text-slate-900 placeholder-slate-400 focus:border-violet-500 shadow-sm'
            }
            ${className}
        `}
    />
...
const StarRating = ({ rating, onChange, isDarkMode }: { rating: number, onChange: (val: number) => void, isDarkMode: boolean }) => {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`text-xs font-bold transition-opacity duration-200 ${currentDisplay > 0 ? 'opacity-100' : 'opacity-0'} ${
                             ...
                             <div className="col-span-2">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute bottom-3 right-3 flex gap-2">
                                        <button className={`p-2 rounded-lg transition-colors flex items-center gap-2 text-xs font-semibold ${isDarkMode ? 'hover:bg-[#27272a] text-zinc-400' : 'hover:bg-slate-100 text-slate-500'}`}>
                                ...
                                <div className="grid grid-cols-4 gap-4">
                                     {['Win', 'Loss', 'BE', 'Pending'].map(res => (
                                         <button
                                            ...
                                            className={`py-4 px-2 rounded-xl border font-bold text-sm transition-all flex flex-col items-center gap-2 ${
                                                formData.result === res 
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                             ...
                             <div className="pt-2">
                                ...
                                <StyledTextArea 
                                    ...
                                    value={formData.exitComment}
                                    onChange={(e: any) => handleInputChange('exitComment', e.target.value)}
                                />
                             </div>
                                 ...
                                 <div>
                                     ...
                                     <div 
                                        className={`w-full h-32 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-3 transition-all overflow-hidden relative ${
                                         isDarkMode ? 'border-[#27272a] bg-[#18181b]/50 hover:border-violet-500 hover:bg-[#18181b]' : 'border-slate-300 bg-slate-50 hover:border-violet-500 hover:bg-slate-100'
                                     }`}>
                                         {screenshots.before ? (
                                             <>
            ...
            `}>
                ...
                <div className="p-7 space-y-7 relative">
                    <div className="flex justify-between items-start">
                         ...
                         <div className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest border shadow-sm ${
                             formData.direction === 'Long' 
      ...
      <div className={`p-6 shrink-0 flex justify-between items-center z-20 relative ${isDarkMode ? 'bg-[#09090b]' : 'bg-white border-slate-200'}`}>
         ...
         <button 
           ...
           className={`px-6 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all ${
             step === 1 ? 'opacity-0 pointer-events-none' : 'hover:bg-slate-100 dark:hover:bg-[#27272a] text-zinc-500'
           }`}
         >
         ...
         {step < 3 ? (
             <button 
                onClick={nextStep}
                className={`px-8 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 active:shadow-md ${
                    isDarkMode ? 'bg-zinc-100 text-black hover:bg-white' : 'bg-slate-900 text-white hover:bg-slate-800'
                }`}
             >
             ...
             <button 
                onClick={finalizeSave}
                className="flex items-center gap-2 px-8 py-3 bg-violet-600 hover:bg-violet-500 text-white rounded-xl text-sm font-semibold shadow-lg shadow-violet-500/20 transition-all active:scale-95"
            >
