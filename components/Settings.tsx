...
const Settings: React.FC<SettingsProps> = ({ isDarkMode, userProfile, onUpdateProfile, onLogout, onToggleTheme }) => {
  ...
  const inputClasses = `w-full bg-transparent border-b-2 py-3 text-lg font-bold outline-none transition-all ${
    isDarkMode 
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center gap-6 mb-12">
                   <div className="relative group">
                      <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white shadow-2xl">
                         <User size={48} />
                      </div>
                      <button className="absolute -bottom-2 -right-2 p-2 bg-[#FF4F01] text-white rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                         <Camera size={16} />
                      </button>
                   </div>
                   <div>
                 ...
                 <div className="grid grid-cols-1 gap-10">
                    ...
                    <div className="group">
                        ...
                        <div className="flex items-center gap-3 pt-3">
                           <span className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${formData.syncMethod === 'EA_CONNECT' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-zinc-500/10 text-zinc-500 border-zinc-500/20'}`}>
                              {formData.syncMethod}
                           </span>
